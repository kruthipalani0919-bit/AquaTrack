...
export default function FarmSetup() {
  ...
  useEffect(() => {
    async function loadFarmDetails() {
      ...
      try {
        ...
        // Farm not found yet, normal for new user initial setup
        setIsCompleted(false);
      } finally {
        setIsLoading(false);
      }
  ...
  const onSubmit = async (data) => {
    ...
    // Payload matching backend model
    const farmPayload = {
  ...
  const currentFarmName = getValues('farmName') || existingFarmData?.farmName || '';
  const currentOwnerName = getValues('ownerName') || existingFarmData?.ownerName || '';
  ...
  return (
    <div className="min-h-screen bg-background text-text-primary font-sans flex flex-col">
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto px-4 py-8 sm:py-12">
        ...
        {/* Async Loading View */}
        {isLoading ? (
          <Card padding="relaxed" className="shadow-lg border-border/80 bg-surface py-16 flex flex-col items-center justify-center text-center">
            <Loader size="lg" color="primary" />
            <span className="text-xs font-medium text-text-secondary mt-3">Verifying farm registration...</span>
          </Card>
        ) : (
          <>
                ...
                <div className="py-8 px-2 sm:px-6 flex flex-col items-center text-center space-y-6 animate-in fade-in duration-200">
                  ...
                  <div className="flex flex-col items-center space-y-2">
                    ...
                    <p className="text-xs sm:text-sm text-text-secondary max-w-md">
                      Your farm {currentFarmName ? <strong className="text-text-primary">"{currentFarmName}"</strong> : ''} has been created and configured.
                    </p>
                  </div>
                  ...
                  <div className="w-full max-w-md bg-background border border-border rounded-xl p-5 shadow-2xs text-left space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs pt-1">
                      ...
                      <div className="bg-surface p-3 rounded-lg border border-border/50">
                        ...
                        <span className="text-sm font-bold text-text-primary mt-0.5 block truncate">
                          {currentOwnerName || 'Not specified'}
                        </span>
                      </div>
                    </div>
                  </div>

                  {/* Dual Action Buttons (Edit Setup & Go to Dashboard) */}
                  <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3 w-full max-w-md">
                    <Button
                      ...
                      className="w-full sm:w-auto font-semibold px-6 py-2.5 text-sm"
                    >
                      Edit Farm Setup
                    </Button>
                    ...
                    <Button
                      ...
                      iconPosition="right"
                      className="w-full sm:w-auto font-semibold px-8 py-2.5 text-sm"
                    >
                      Go to Dashboard →
                    </Button>
                  </div>
