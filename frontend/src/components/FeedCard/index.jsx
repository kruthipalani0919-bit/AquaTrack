...
import { UtensilsCrossed, Container, Sprout, Calendar, Clock, IndianRupee, Eye, Edit3, Trash2, Package } from 'lucide-react';
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 pb-3 border-b border-border/60">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="font-bold text-sm sm:text-base text-text-primary truncate tracking-tight" title={feedBrand}>
              {feedBrand} <span className="font-normal text-text-secondary">({feedType})</span>
            </h3>
            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
              <span className="text-[11px] text-text-secondary flex items-center gap-1 font-medium">
                ...
                <span className="truncate max-w-[120px]">{cropName}</span>
              </span>
              <span className="text-[11px] text-text-secondary flex items-center gap-1 font-medium">
      ...
      <div className="grid grid-cols-3 gap-2 py-4 border-b border-border/60 text-center">
        <div className="flex flex-col items-center p-2 rounded-lg bg-background/60 border border-border/40">
          ...
          <span className="text-xs sm:text-sm font-bold text-text-primary mt-0.5">
            {quantityKg} <span className="text-[10px] font-normal text-text-secondary">Kg</span>
          </span>
        </div>
        ...
        <div className="flex flex-col items-center p-2 rounded-lg bg-background/60 border border-border/40">
          ...
          <span className="text-xs sm:text-sm font-bold text-text-primary mt-0.5">
            {remainingStockKg} <span className="text-[10px] font-normal text-text-secondary">Kg</span>
          </span>
        </div>
      </div>
      ...
      <div className="py-3 flex items-center justify-between text-xs text-text-secondary border-b border-border/40">
        <span className="flex items-center gap-1 font-medium">
          <Calendar className="w-3.5 h-3.5 text-primary" /> {feedingDate}
        </span>
        <span className="flex items-center gap-1 font-medium">
          <Clock className="w-3.5 h-3.5 text-accent" /> {feedingTime}
        </span>
      </div>
